...
// MAIN – full reset of state + clear first line
// ***** Logan Walters *****
(MAIN)
    // reset input flags
    @0
...
// INPUT: reads keys, draws 0/1, supports backspace, C/c, Q/q, Enter
// ***** Logan Walters *****
(INPUT_LOOP)
...
// CLEAR: hard reset – screen line, registers, decimal state
// ***** Logan Walters *****
(HANDLE_CLEAR)
    @CLEAR_ALL
    0;JMP
...
// PROCESS – build signed value from R0..R15 and draw
// R0 is sign bit (MSB), R15 is LSB.
// For positives: value = normal.
// For negatives: valueMagnitude = -rawValue (Hack 16-bit 2's complement).
// ***** Aspen Cristobal *****
(PROCESS)
...
// SPLIT_DECIMAL – val -> decD4..decD0 (simple repeated sub)
// val holds the magnitude (always non-negative here).
// ***** Aspen Cristobal *****
(SPLIT_DECIMAL)
    // clear digits
    @0
...
// DRAW_RESULT – prints " --> value" or " --> -value" after bits
// Arrow is exactly: "-->" (two dashes then greater-than).
// No '+' sign for positives; '-' only for negatives.
// ***** Aspen Cristobal *****
(DRAW_RESULT)
...
// FONT: ge_continue_output + digits 0..9, space, '-', 'g', '+'
